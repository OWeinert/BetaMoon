-- An event subscription runs later when something happens, rather than during registration.
-- The function receives an event-specific table: fields from one event may not exist in another.
-- Most handlers below only read values to document what is available. Break a block
-- to see the block_broken handler actually send a chat message.
-- These are global notifications; the later interaction examples use callbacks owned by content.
...
function modInit()
  ...
  -- Tick events are frequent. Keep their handlers small, and subscribe
  -- once during modInit rather than registering a new handler every tick.
