...
public final class BlockPlacedHook implements HookModule {
    ...
    private static final String CALLBACK_OWNER = "betamoon/instrumentation/hooks/block/BlockPlacedCallbacks";
    private static final String SNAPSHOT_DESCRIPTOR = "Lbetamoon/instrumentation/hooks/block/BlockSnapshot;";
    ...
    public void register(HookRegistrar registrar) {
        ...
        HandlerRef completed = HandlerRef.of(CALLBACK_OWNER, "afterBlockPlaced", "(Z" + SNAPSHOT_DESCRIPTOR + ")V");

        registrar.register(AroundHookDefinition.builder(ID, BlockHookTargets.SEND_PLACE_BLOCK)
                .capture(capture, ValueBinding.instanceField(BlockHookTargets.PLAYER_CONTROLLER_MC),
                        ValueBinding.argument(1), ValueBinding.argument(3), ValueBinding.argument(4),
                        ValueBinding.argument(5), ValueBinding.argument(6))
                .onReturn(completed, ValueBinding.returnValue(), ValueBinding.capturedValue()).build());
    }
}
