...
public final class LuaContentRegistry {
    private static final Map<String, Entry> CONTENT = new HashMap<>();
    ...
    private static String key(String namespace, int id) {
        ...
        if (owner == null) {
            throw new LuaError("Content registration is only available while a script is loading.");
        }
        return owner + "\n" + namespace + "\n" + id;
    }

    public static synchronized Entry find(String namespace, int id) {
        Entry entry = CONTENT.get(key(namespace, id));
        if (entry != null) {
            entry.seen = true;
        }
        return entry;
    ...
    public static synchronized Entry remember(String namespace, int id, Object value, String kind) {
        ...
        Entry entry = CONTENT.get(key);
        if (entry == null) {
            String owner = LuaScriptRegistry.getCurrentScriptFile();
            entry = new Entry(value, kind, owner, namespace, id);
            CONTENT.put(key, entry);
        } else if (!entry.kind.equals(kind)) {
            throw new LuaError("Cannot hot-reload " + namespace + " " + id + " from type '" + entry.kind + "' to '"
                    + kind + "'. Restart Minecraft to change its type.");
        }
        return entry;
    }
    ...
    /**
     * Returns the declaring script for a registered object, or null for foreign
     * content.
     */
    ...
    public static synchronized List<String> finishLoadPass() {
