...
/**
 * Generated Minecraft container backed by a standalone Lua container
 * definition.
 */
public final class LuaContainer extends Container {
    ...
    public LuaContainer(InventoryPlayer player, LuaTileEntity entity, ContainerDefinition definition) {
        ...
        for (int row = 0; row < 3; row++) {
            for (int column = 0; column < 9; column++) {
                addSlot(new Slot(player, column + row * 9 + 9, definition.playerX + column * 18,
                        definition.playerY + row * 18));
            }
        }
        if (definition.includeHotbar) {
            for (int column = 0; column < 9; column++) {
                addSlot(new Slot(player, column, definition.playerX + column * 18, definition.playerY + 58));
            }
        }
    ...
    public boolean isUsableByPlayer(EntityPlayer player) {
        return entity.canInteractWith(player);
    }
    ...
    public void updateCraftingResults() {
        ...
        for (int fieldIndex = 0; fieldIndex < syncedFields.size(); fieldIndex++) {
            int value = entity.getDataInt(syncedFields.get(fieldIndex));
            if (lastValues.get(fieldIndex).intValue() == value) {
                continue;
            }
    ...
    public void func_20112_a(int id, int value) {
        if (id >= 0 && id < syncedFields.size()) {
            entity.setSyncedData(syncedFields.get(id), value);
        }
    }
    ...
    public ItemStack getStackInSlot(int index) {
        ...
        if (index < tileSlotCount) {
            func_28125_a(source, tileSlotCount, slots.size(), true);
        } else {
            transferToTile(source);
        }
    ...
    private void transferToTile(ItemStack source) {
        ...
        for (int i = 0; i < tileSlotCount && source.stackSize > 0; i++) {
            Slot target = (Slot) slots.get(i);
            if (!target.isItemValid(source) || target.getHasStack()) {
                continue;
            }
    ...
    private static final class OutputSlot extends Slot {
        private OutputSlot(LuaTileEntity inventory, int index, int x, int y) {
