...
public final class ToolMaterialRegistry {
    private ToolMaterialRegistry() {
    }

    public static EnumToolMaterial register(String name, int harvestLevel, int maxUses, float efficiency, int damage) {
    ...
    /**
     * Allocates a new {@link EnumToolMaterial} instance without invoking the enum
     * constructor. This uses the JDK's {@link ReflectionFactory} to build a
     * serialization constructor, then manually injects the enum core fields
     * (name/ordinal) and the material stats fields. This is required because enum
     * constructors cannot be called directly at runtime.
     *
     * @param name
     *            The enum constant name to assign (already uppercased).
     * @param ordinal
     *            The enum ordinal to assign (typically current values length).
     * @param harvestLevel
     *            The harvest level to store on the created material.
     * @param maxUses
     *            The durability value to store on the created material.
     * @param efficiency
     *            The mining efficiency value to store on the created material.
     * @param damage
     *            The base damage value to store on the created material.
     * @return A fully populated {@link EnumToolMaterial} instance not yet
     *         registered in the enum values array.
     */
     ...
     * This is used to inject enum core fields (name/ordinal) and tool material
     * stats.
     *
     * @param owner
     *            The class declaring the field to be set.
     * @param target
     *            The instance to mutate (the new enum constant).
     * @param fieldName
     *            The declared field name to update.
     * @param value
     *            The value to write into the field.
     * @throws Exception
     *             If reflection access fails or the field does not exist.
     */
     ...
     * non-static fields and verify their expected type order: (int harvestLevel,
     * int maxUses, float efficiency, int damage). This ensures the new enum
     * constant has valid stats even when field names differ between MCP and
     * runtime.
     *
     * @param target
     *            the EnumToolMaterial instance to populate
     * @param harvestLevel
     *            mining harvest level to assign
     * @param maxUses
     *            durability value to assign
     * @param efficiency
     *            mining efficiency value to assign
     * @param damage
     *            base damage value to assign
     * @throws Exception
     *             if the fields cannot be located or updated
     */
    private static void setMaterialFields(EnumToolMaterial target, int harvestLevel, int maxUses, float efficiency,
    ...
    /**
     * Replaces the hidden enum values array for the given enum class. Java enums
     * cache the array returned by {@code values()} in a synthetic static field.
     * This method finds that field, removes {@code final}, and writes the new array
     * so future {@code values()} calls include the new material.
     *
     * @param enumClass
     *            The enum class whose values array should be replaced.
     * @param values
     *            The new values array containing the appended material.
     */
    ...
    /**
     * Clears the internal Class<?> caches for enum constants. Without this,
     * {@code Enum.valueOf(..)} and related lookups may still use stale cached data
     * and fail to find the newly injected constant.
     *
     * @param enumClass
     *            The enum class whose caches should be cleared.
     */
    ...
    /**
     * Removes the {@code final} modifier from a {@link Field} so it can be
     * reassigned. This is required when mutating enum internals and the synthetic
     * enum values array.
     *
     * @param field
     *            The field whose modifiers should be updated.
     * @throws Exception
     *             If the modifiers field cannot be accessed or updated.
     */
