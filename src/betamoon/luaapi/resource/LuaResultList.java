...
/**
 * A query result that is both a normal 1-based Lua array and a small collection
 * API.
 */
public final class LuaResultList extends LuaTable {
    ...
    public LuaResultList(List<? extends LuaValue> values, BulkOverride bulkOverride) {
        this.bulkOverride = bulkOverride;
        for (int i = 0; i < values.size(); i++) {
            set(i + 1, values.get(i));
        }
        set("first", new Select(this, false));
        set("last", new Select(this, true));
        set("one", new One(this));
        set("isEmpty", new IsEmpty(this));
        if (bulkOverride != null) {
            set("overrideAll", new OverrideAll(this));
        }
    }
    ...
    private static final class Select extends VarArgFunction {
        ...
        private Select(LuaResultList list, boolean last) {
            this.list = list;
            this.last = last;
        }
    ...
    private static final class One extends VarArgFunction {
        private final LuaResultList list;
        private One(LuaResultList list) {
            this.list = list;
        }
        ...
        public Varargs invoke(Varargs args) {
            ...
            if (length != 1) {
                throw new LuaError("Expected exactly one result, found " + length + ".");
            }
            return list.get(1);
        }
    }
    ...
    private static final class IsEmpty extends VarArgFunction {
        private final LuaResultList list;
        private IsEmpty(LuaResultList list) {
            this.list = list;
        }

        public Varargs invoke(Varargs args) {
            return valueOf(list.length() == 0);
        }
    }

    private static final class OverrideAll extends VarArgFunction {
        private final LuaResultList list;
        private OverrideAll(LuaResultList list) {
            this.list = list;
        }
        ...
        public Varargs invoke(Varargs args) {
            ...
            if (!definition.istable()) {
                throw new LuaError("overrideAll expects an override table.");
            }
